...
class My_Neural_Network():
    ...
    def backward_propagation(self, expected):
        self.weights[-1] = self.weights[-1] - (self.alpha * self.__output_layer_partial_derivatives(expected))
        self.bias[-1] = self.bias[-1] - (self.alpha * derivative_delta_output_layer(self.predicted, expected))
        for i in range(len(self.weights) - 2, -1, -1): #range starts from last non-output weights until first weights (index 0)
            self.weights[i] = self.weights[i] - (self.alpha * self.__deep_layer_partial_derivatives(i, expected))
            self.bias[i] = self.bias[i] - (self.alpha * self.__delta_derivative_deep_layer(i, expected))
